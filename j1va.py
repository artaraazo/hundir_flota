...
def paint(sender,word,state):
    ...
    screen.fill(BLACK)
    pygame.display.set_caption("Tableros jugador1")
    for row in range(41):
...
for (i,boat) in boats:
    ...
    if z=="v":
        ...
        messages.append("jugador1,b" + str(n) +","+ letter +str(x))
        i-=1
        while i>0:
            letter = chr(ord(y)+(i-1))
            messages.append("jugador1,b" + str(n) +","+ letter +str(x))
    ...
    if z=="h":
        ...
        messages.append("jugador1,b" + str(n) +","+ y+str(x+(i-1)))
        i-=1
        while i>0:
            messages.append("jugador1,b" + str(n) +","+ y+str(x+(i-1)))
            if i==1:
